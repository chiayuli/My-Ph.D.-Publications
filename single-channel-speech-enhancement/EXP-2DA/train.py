...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            cyclegan_loss=total_DA1_loss+total_DA2_loss+total_DB_loss+\
        ...
        print("### fake_B ###")
        print(model.fake_B)
        print("Epoch %d total loss: %.5f, DA1: %.5f, DA2: %.5f ,DB: %.5f, GA: %.5f, GB: %.5f " % (epoch, total_loss, total_DA1_loss, total_DA2_loss, total_DB_loss, total_GA_loss, total_GB_loss))
