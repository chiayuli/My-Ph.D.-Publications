...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        print("In UnalignedDataset ", opt.dataroot)
        if opt.phase == 'train':
            self.feat_A = os.path.join(opt.dataroot, opt.phase + 'A/feats.scp')
            self.feat_B = os.path.join(opt.dataroot, opt.phase + 'B/feats.scp')
        elif opt.phase == 'test':
            self.feat_A = os.path.join(opt.dataroot+'/feats.scp')
            self.feat_B = os.path.join(opt.dataroot.replace("testA","testB")+ '/feats.scp')
            print("2 ", opt.dataroot)
            print(self.feat_A)
        elif opt.phase == 'oldtest':
            self.feat_A = os.path.join(opt.dataroot+ '/testA/feats.scp')
            self.feat_B = os.path.join(opt.dataroot+ '/testB/feats.scp')
