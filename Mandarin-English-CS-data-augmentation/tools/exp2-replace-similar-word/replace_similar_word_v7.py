...
def augment3(line, translator, length):
    words = re.findall(r'[a-z]+|[\u4e00-\ufaff]+', line, re.UNICODE)
    strings = [line]
    for end in range(4,9):
        cmi=round(random.randint(3,int(end))*0.1, 2)
        print('cmi '+str(cmi))
        if len(words) < length:
            # the number of ZH word needs to be replaced by English word
            num = int(round((len(words)*cmi)))
            print('EN num '+str(num)+', len(words) '+str(len(words)))
            indexs = random.sample(range(0, len(words)), num)
            print(str(indexs))
            for i in range(num):
                #index = random.randint(1,100) % len(words)
                if re.match(r'[a-z]+', words[indexs[i]]):
                    target='zh-CN'
                else:
                    target='en'
                translation = translator.translate(words[indexs[i]],target_language=target)
                words[indexs[i]] = translation['translatedText'].lower()
...
if __name__ == '__main__':
    ...
    with open(args.text, 'r') as f:
        with open(args.output, 'w') as out:
            for line in f:
                print('-----------------')
                strings, cmi=augment3(line, translate_client, int(args.length))
                print(len(strings))
                for i in range(len(strings)):
                  if i == 0:
                    print('Original: '+strings[i])
                  else:
                    print('Synthetic: '+str(cmi)+' '+strings[i])
                    out.write(strings[i])
