...
def augment3(line, translator, length):
    ...
    for end in range(1):
        cmi=round(random.randint(3,5)*0.1, 2)
        if len(words) < 20:
            ...
            if num >= len(words)+2:
              num = len(words) - 1
            indexs = random.sample(range(1, len(words)), num)
            if len(indexs) == 0:
              indexs.append(1)
            print('indexs: '+str(indexs))
            new_words = lineSOSEOS.split()
            for idx in indexs:
                if re.match(r'[a-z]+', new_words[idx]):
                    target='zh-CN'
                else:
                    target='en'
                translation = translator.translate(new_words[idx],target_language=target)
