...
train_config=conf/tuning/train_rnn_v2_bs15.yaml
lm_config=conf/lm.yaml
decode_config=conf/tuning/decode_rnn_v2.yaml
...
## loss
unsupervised_loss=mmd # mmd (original inter-domain paper), idt, idt_cycdom_t2t, cycdom_t2t 
sup_loss_ratio=0.5
st_ratio=0
use_batchnorm=False
weight_decay=0.0
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    echo "stage 0 (skip, use asr1/data) : Data preparation"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for rtask in ${recog_set}; do
        ...
        dump.sh --cmd "$train_cmd" --nj 4 --do_delta ${do_delta} \
            data/${rtask}/feats.scp data/${unpaired_set}/cmvn.ark exp/dump_feats/recog/${rtask} \
            ${feat_recog_dir}
    done
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    local/filtering_samples.py \
        --config ${train_config} \
        --preprocess-conf ${preprocess_config} \
        --data-json ${feat_us_dir}/data.json \
        --mode-subsample "asr" \
        ${min_io_delta:+--min-io-delta $min_io_delta} \
        --output-json-path ${feat_us_dir}/data.json
fi

expname=semi_${loss}_${train_set}_${backend}
expdir=exp/$expname/init
mkdir -p $expdir
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --tensorboard-dir tensorboard/${expname}_init \
        --debugmode ${debugmode} \
        --dict ${dict} \
        --debugdir ${expdir} \
        --minibatches ${N} \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! ${skip_lm_training}; then
    ...
    if [ ${use_wordlm} = true ]; then
        ...
        cut -f 2- -d" " data/${recog_set}/text > ${lmdatadir}/test.txt
        cat ${lmdatadir}/train_trans.txt ${lmdatadir}/train_others.txt > ${lmdatadir}/train.txt
        text2vocabulary.py -s ${lm_vocabsize} -o ${lmdict} ${lmdatadir}/train.txt
    else
        lmdatadir=data/local/lm_train_larger
        lmdict=${dict}
        mkdir -p ${lmdatadir}
        text2token.py -s 1 -n 1 -l ${nlsyms} data/${unpaired_set}/text \
        ...
        text2token.py -s 1 -n 1 -l ${nlsyms} data/${recog_set}/text \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Decoding Init-Network"
    nj=32
    ...
    (
        ...
        if ${skip_lm_training}; then
            ...
            if [ ${use_wordlm} = true ]; then
                ...
                lmtag=charlm_larger
                lmexpdir=exp/train_rnnlm_pytorch_charlm_larger
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --model ${expdir}/init_results/${recog_model}  \
            ${recog_opts}
...
if [[ "$unsupervised_loss" == "mmd" ]]; then
    ver=6
elif [[ "$unsupervised_loss" == "idt_cycdom_t2t" ]];
    ver=5
elif [[ "$unsupervised_loss" == "cycdom_t2t" ]];
    ver=7
elif [[ "$unsupervised_loss" == "idt" ]];
    ver=9
else
    echo "Unknown unsupervised loss, exit..."
    exit 1;
fi

retrain_script=asr_retrain${ver}.py
decode_script=unsupervised_recog${ver}.py
module="espnet.nets.pytorch_backend.unsupervised${ver}:E2E"
init=${expdir}/results/model.acc.best
expdir=exp/$expname/retrain_str${st_ratio}_${unsupervised_loss}_max
mkdir -p $expdir

if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    echo "stage 6: Network Re-Training on unpaired data"
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        asr_retrain.py \
        --config ${train_config} \
        --preprocess-conf ${preprocess_config} \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    echo "stage 7: Re-trained Network Decoding"
    nj=32
    pids=() # initialize pids
    for rtask in ${recog_set}; do
    (
