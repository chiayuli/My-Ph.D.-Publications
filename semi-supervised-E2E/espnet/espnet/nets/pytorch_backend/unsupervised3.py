...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args):
       ...
       # subsample info
        ...
        self.enc_common_rnn0 = getattr(self.enc.enc[0], "birnn%d" % (args.elayers-2))
        self.enc_common_merge0 = getattr(self.enc.enc[0], "bt%d" % (args.elayers-2))
        # ctc
    ...
    def forward_common(self, xpad, xlen):
        ...
        xpack, states = self.enc_common_rnn0(xpack)
        xpad, xlen = pad_packed_sequence(xpack, batch_first=True)
        b, t, o = xpad.shape
        #logging.warning("xpad2 %d %d %d"%(xpad2.shape))
        xpack2 = self.enc_common_merge0(xpad.contiguous().view(b * t, o)).view(b, t, -1)
        xpack2 = pack_padded_sequence(xpack2, xlen, batch_first=True)
        #logging.warning("xpack2 %d %d %d"%(xpad2.shape))
        hpack, states = self.enc_common_rnn(xpack2)
        hpad, hlen = pad_packed_sequence(hpack, batch_first=True)
        b, t, o = hpad.shape
        #logging.warning("hpad %d %d %d"%(hpad.shape))
        hpad = torch.tanh(self.enc_common_merge(hpad.contiguous().view(b * t, o)).view(b, t, -1))
        #logging.warning("After hpad %d %d %d"%(hpad.shape))
        return hpad, hlen
    ...
    def forward(self, xs_pad, ilens, ys_pad, olens, supervised=False, discriminator=None, only_encoder=False):
        ...
        if supervised or not self.training:
            ...
            #for name, layer in self.enc_common_rnn2.named_modules():
            ...
            if True:
                ### high-level representation from text
                ...
                ### high-level representation from speech
            ...
            #loss = (self.speech_text_ratio * (loss_cyc_h_from_x+loss_idt_enc_s.mean())) +\
            #       ((1.0 - self.speech_text_ratio) * (loss_text+loss_cyc_h_from_y.mean()+loss_idt_enc_t.mean()))
