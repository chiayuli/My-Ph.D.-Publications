...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward_common(self, xpad, xlen):
        ...
        hpack2, states = self.enc_rnn2(xpack)
        ...
        hpack, states = self.enc_rnn4(hpack3)
        hpad, hlen = pad_packed_sequence(hpack, batch_first=True)
        b, t, o = hpad.shape
        hpad = torch.tanh(self.enc_merge4(hpad.contiguous().view(b * t, o)).view(b, t, -1))
        xpack = pack_padded_sequence(hpad, hlen, batch_first=True)
    ...
    def forward(self, xs_pad, ilens, ys_pad, olens, supervised=False, discriminator=None, only_encoder=False):
        ...
        if supervised or not self.training:
            ...
            if True:
                ...
                ys_hat_pad = pad_list(ys_hat, 0)
                hypad2, hylens2 = self.enc_t(ys_hat_pad, hylens)
                loss_e_cyc_i_from_y = (hypad - hypad2)**2
            ...
            loss = (self.speech_text_ratio * (loss_cyc_h_from_x+loss_idt_enc_s.mean()+loss_e_cyc_i_from_x)) +\
                   ((1.0 - self.speech_text_ratio) * (loss_cyc_h_from_y.mean()+loss_idt_enc_t.mean()+loss_e_cyc_i_from_y.mean()))
