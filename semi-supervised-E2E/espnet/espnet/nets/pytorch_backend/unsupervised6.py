    ...
    def forward(self, xs_pad, ilens, ys_pad, olens, supervised=False, discriminator=None, only_encoder=False):
        ...
        if supervised or not self.training:
            ...
            if self.unsupervised_loss is not None and self.unsupervised_loss != "None":
                ...
                #logging.warning("unsupervised_loss: %s" %(self.unsupervised_loss))
            ...
            # 3. forward decoders
            loss_text, acc, att_t = self.dec(hypad, hylens, ys_pad)
            #logging.warning("loss_hidden %f loss_text %f acc %f" %(loss_unsupervised, loss_text, acc))
