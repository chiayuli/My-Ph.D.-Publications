...
from espnet.nets.pytorch_backend.unsupervised5_tSNE import E2E, Discriminator
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        x, il, y, ol = _recursive_to(batch, self.device)
        u_x, u_il, u_y, u_ol = _recursive_to(u_batch, self.device)
        ...
        if self.ngpu == 1:
          ...
          loss_unsupervised = self.model(u_x,u_il,u_y,u_ol, False).mean() / self.accum_grad
            ...
            # apex does not support torch.nn.DataParallel
            loss_supervised = (
                data_parallel(self.model, (x, il, y, ol, True), range(self.ngpu)).mean() / self.accum_grad
            )
            loss_unsupervised = (
                data_parallel(self.model, (u_x, u_il, u_y, u_ol, False), range(self.ngpu)).mean() / self.accum_grad
            )
        #logging.warning("loss_supervised %f loss_unsupervised %f" %(loss_supervised, loss_unsupervised))
        optimizer.zero_grad()
...
def printtSNE(args):
    ...
    load_inputs_and_targets_2 = LoadInputsAndTargets(
        mode="asr",
        load_output=True,
    ...
    if args.batchsize == 0:
        ...
        with torch.no_grad():
            for names in grouper(args.batchsize, keys, None):
                ...
                xs_pad, ilens, ys_pad, olens = converterU([load_inputs_and_targets_2(batch)])
                if args.streaming_mode == "window" and args.num_encs == 1:
                    raise NotImplementedError
                elif args.streaming_mode == "segment" and args.num_encs == 1:
                    if args.batchsize > 1:
                    ...
                    logging.warning("1268 model.recognize_batch()...")
                    #nbest_hyps = model.recognize_batch(
                    #    feats, args, train_args.char_list, rnnlm=rnnlm
                    #)
                    hs_pad, hy_pad = model.recognize_batch(
                        feats, ys_pad, olens, args, train_args.char_list, rnnlm=rnnlm
                    )
                    logging.warnign("1275: got it")
