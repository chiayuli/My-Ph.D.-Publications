...
max_active=20000 # max-active
beam=50.0 # beam used
latbeam=15.0 # beam used in getting lattices
...
##
finalfeats="$feats nnet-forward --class-frame-counts=$dir/class.counts --apply-log=true --no-softmax=false $srcdir/dnn.nnet.5t2_rml ark:- ark:- |"
...
if ! $skip_scoring ; then
  [ ! -x local/score.sh ] && \
    echo "$0: not scoring because local/score_ihm.sh does not exist or not executable." && exit 1;
