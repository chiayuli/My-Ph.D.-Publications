...
if [ $# != 4 ]; then
   ...
   echo "usage: steps/build_nnet_pfile.sh <data-dir> <ali-dir> <exp-dir> <number-skips>"
   echo "e.g.:  steps/build_nnet_pfile.sh data/train exp/tri4_ali exp/tri4_pfile 1"
...
if ! which pfile_create >/dev/null; then # pfile_create not on our path.
  ...
  try_path=$KALDI_ROOT/tools/pfile_utils-v0_51/
...
if [ $stage -le 2 ]; then
  $cmd JOB=1:1 $dir/log/build_pfile.$name.JOB.log \
  ...
  if $do_concat; then
    ...
    $cmd JOB=3:3 $dir/log/gzip.$name.JOB.log gzip $dir/$name.pfile.JOB || exit 1;
  fi
fi
