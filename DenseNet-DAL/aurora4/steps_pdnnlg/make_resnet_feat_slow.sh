...
$cmd JOB=1:3 $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 \; \
    python pdnnlg/cmds2/run_FeatExt_Kaldi_resnet.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index

#$cmd JOB=3:4 $logdir/conv_feat.$name.JOB.log \
#    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
#    export THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 \; \
#    python pdnnlg/cmds2/run_FeatExt_Kaldi_resnet.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index
$cmd JOB=4:6 $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 \; \
    python pdnnlg/cmds2/run_FeatExt_Kaldi_resnet.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index

#$cmd JOB=7:8 $logdir/conv_feat.$name.JOB.log \
#    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
#    export THEANO_FLAGS=mode=FAST_RUN,device=cpu,floatX=float32 \; \
#    python pdnnlg/cmds2/run_FeatExt_Kaldi_resnet.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index
