...
$cmd JOB=1:$nj $logdir/conv_feat.$name.JOB.log \
    ...
    THEANO_FLAGS='mode=FAST_RUN,device=cpu,floatX=float32' python pdnnlg/cmds2/run_FeatExt_Kaldi_DENSENET.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index
