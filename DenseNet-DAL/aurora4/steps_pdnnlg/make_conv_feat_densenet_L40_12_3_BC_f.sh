#!/bin/bash
# Copyright 4014     Yajie Miao     Carnegie Mellon University
# Apache 2.0
...
$cmd JOB=1:2 $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    THEANO_FLAGS='mode=FAST_RUN,device=cpu,floatX=float32' python pdnnlg/cmds2/run_FeatExt_Kaldi_DENSENET_L40_12_3_BC_f.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index
 
$cmd JOB=3:4 $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    THEANO_FLAGS='mode=FAST_RUN,device=cpu,floatX=float32' python pdnnlg/cmds2/run_FeatExt_Kaldi_DENSENET_L40_12_3_BC_f.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index

$cmd JOB=5:6 $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    THEANO_FLAGS='mode=FAST_RUN,device=cpu,floatX=float32' python pdnnlg/cmds2/run_FeatExt_Kaldi_DENSENET_L40_12_3_BC_f.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index

$cmd JOB=7:$nj $logdir/conv_feat.$name.JOB.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnnlg/ \; \
    THEANO_FLAGS='mode=FAST_RUN,device=cpu,floatX=float32' python pdnnlg/cmds2/run_FeatExt_Kaldi_DENSENET_L40_12_3_BC_f.py --in-scp-file $feadir/nnet_input.$name.JOB.scp --out-ark-file $feadir/conv_feats.$name.JOB.ark  --nnet-param $cnnparam --nnet-cfg $cnncfg --layer-index $layer_index
