...
from io_func.model_io_densenet import _file2nnetx, _file2nnety, log
...
if __name__ == '__main__':
    ...
    cfg = cPickle.load(smart_open(nnet_cfg,'r'))
    model = None
    model = DENSENET(cfg = cfg)

    # load model parameters
    wdir = "/mount/arbeitsdaten/asr/licu/kaldi-trunk3/egs/aurora4/s5_1/exp_pdnn/cnn/"
    nnet_param_x = wdir + "nnet.tmp_DENSENET.L61_12_test2_theta0_5_test2_bn_x_rml"
    nnet_param_y = wdir + "nnet.tmp_DENSENET.L61_12_test2_theta0_5_test2_bn_y_rml"
    _file2nnetx(model, filename = nnet_param_x)
    _file2nnety(model, filename = nnet_param_y)
...
#        in_matrix = numpy.reshape(in_matrix, (in_matrix.shape[0],) + input_shape_1)
        print "[Debug] in_matrix.shape ", in_matrix.shape
        out_matrix = extract_func(in_matrix)
        print "[Debug] out_matrix.shape ", out_matrix.shape
