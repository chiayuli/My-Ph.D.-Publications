...
def train_y_sgd(train_y_fn, cfg):
    ...
    while (not train_sets.is_finish()):
        ...
        for batch_index in xrange(train_sets.cur_frame_num / batch_size):  # loop over mini-batches
            acc, cost, y, y_pred= train_y_fn(index=batch_index, learning_rate = learning_rate)
            #print "y_cost: ", cost
...
def train_z_sgd(train_z_fn, cfg):
    ...
    while (not train_sets.is_finish()):
        ...
        #print "train_sets.cur_frame_num ", train_sets.cur_frame_num
        for batch_index in xrange(train_sets.cur_frame_num / batch_size):  # loop over mini-batches
            acc, cost, z, z_pred = train_z_fn(index=batch_index, learning_rate = learning_rate)
            #print "z_cost: ", cost
            train_acc.append(acc)
            train_cost.append(cost)
            print "z:"
...
def train_x_sgd(train_x_fn, cfg):
    ...
    while (not train_sets.is_finish()):
        ...
        #print "train_sets.cur_frame_num ", train_sets.cur_frame_num
        for batch_index in xrange(train_sets.cur_frame_num / batch_size):  # loop over mini-batches
            acc, acc2, cost = train_x_fn(index=batch_index, learning_rate = learning_rate)
            train_acc.append(acc)
            train_acc2.append(acc2)
            train_cost.append(cost)
    train_sets.initialize_read()
    return train_acc, train_acc2, train_cost
