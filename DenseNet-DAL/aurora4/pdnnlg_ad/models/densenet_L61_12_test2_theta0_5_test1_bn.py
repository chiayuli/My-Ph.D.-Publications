...
class DENSENET(object):
    ...
    def __init__(self, cfg = None, testing = False, input = None):
        ...
        self.x_finetune_cost = self.y_finetune_cost - (1*self.z_finetune_cost)        
    ...
    def build_x_network(self, input_var=None):                
        ...
        n = (self.x_depth - 1) // self.x_num_blocks
        for b in range(self.x_num_blocks):
            ...
            print "[Debug] x_network net_shape: ", b, net_shape_tmp
        self.x_shape = net_shape_tmp
        return layer
    ...
    def build_y_network(self, input_var=None):
        ...
        n = (self.y_depth) // self.y_num_blocks
        for b in range(self.y_num_blocks):
            layer = self.dense_block(layer, n - 1, self.growth_rate, self.dropout, self.y_layers)
            if b < self.y_num_blocks - 1:
                t_n = int(layer.output_shape[1] * self.theta)
                layer = self.transition(layer, t_n, self.dropout, self.y_layers)

            net_shape_tmp = lasagne.layers.get_output_shape(layer)
            print "[Debug] y_network net_shape: ", b, net_shape_tmp
    ...
    def build_z_network(self, input_var=None):           
        input_size = self.x_shape[1]*self.x_shape[2]*self.x_shape[3]     
        layer = InputLayer(shape=(None, input_size), input_var=input_var)                
        # fully-connected layer                
        layer = ReverseGradientLayer(layer , 2)                
        layer = DenseLayer(layer, 512, nonlinearity=sigmoid)                
        self.z_layers.append(layer_info('fc', (input_size, 512), 2))
        layer = DenseLayer(layer, self.C_classes, nonlinearity=softmax)                
        self.z_layers.append(layer_info('fc', (512, self.C_classes), 2))
        return layer   
