...
class DENSENET(object):
    ...
    def build_y_network(self, input_var=None):
        input = input_var.reshape((-1, self.x_shape[1]))
        layer = InputLayer(shape=(None, self.x_shape[1]), input_var=input)

        self.conv_output_dim = self.x_shape[1]
        self.cfg.n_ins = self.x_shape[1]
    ...
    def build_finetune_functions(self, train_shared_xyz, valid_shared_xyz, batch_size):
        ...
        train_x_fn = theano.function(inputs=[index, learning_rate],              
                                        outputs=[self.acc, self.acc2, self.x_finetune_cost, self.y_finetune_cost, self.z_finetune_cost],              
                                        updates=updates_x,              
                                        givens={                
                                            self.x: train_set_x[index * batch_size:                                    
                                                                (index + 1) * batch_size],                
