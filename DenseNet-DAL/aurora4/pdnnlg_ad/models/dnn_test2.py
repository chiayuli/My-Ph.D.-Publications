...
class DNN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None,
        ...
        pp_y = lasagne.layers.get_output(self.y_network, lasagne.layers.get_output(self.x_network))
        pp_z = lasagne.layers.get_output(self.z_network, lasagne.layers.get_output(self.x_network))
        self.y_pred = T.argmax(pp_y, axis=1)        
        self.z_pred = T.argmax(pp_z, axis=1)        
        self.acc = T.mean(T.eq(self.y_pred, self.y), dtype=theano.config.floatX)
        self.acc2 = T.mean(T.eq(self.z_pred, self.z), dtype=theano.config.floatX)
        #self.y_finetune_cost = -T.mean(T.log(pp)[T.arange(self.y.shape[0]), self.y])
        self.y_finetune_cost = lasagne.objectives.categorical_crossentropy(pp_y, self.y).mean()
        self.z_finetune_cost = lasagne.objectives.categorical_crossentropy(pp_z, self.z).mean()
        #self.z_finetune_cost = -T.mean(T.log(pp2)[T.arange(self.z.shape[0]), self.z])
        #alpha = min(self.epoch/10, 1) * 10
        self.x_finetune_cost = self.y_finetune_cost + self.z_finetune_cost
        #self.epoch = self.epoch + 1
    
    def build_x_network(self, input_var=None):        
        #custom_rectify = LeakyRectify(0.0001)
    ...
    def build_y_network(self, input_var=None):        
        #custom_rectify = LeakyRectify(0.9)
        ...
        layer = batch_norm(DenseLayer(layer, 1024, nonlinearity=rectify))        
        layer = batch_norm(DenseLayer(layer, 1024, nonlinearity=rectify))        
        layer = DenseLayer(layer, self.n_outs, nonlinearity=softmax)        
        # project and reshape        
        #layer = DenseLayer(layer, 3*11*40, nonlinearity=None)        
        #layer = ReshapeLayer(layer, ([0], 3, 11, 40))        
    ...
    def build_z_network(self, input_var=None):        
        ...
        layer = DenseLayer(layer, 512, nonlinearity=rectify)        
        #layer = DenseLayer(layer, 1024, nonlinearity=sigmoid)        
    ...
    def build_finetune_functions(self, train_shared_xyz, valid_shared_xyz, batch_size):
        ...
        train_y_fn = theano.function(inputs=[index, learning_rate],
              outputs=[self.acc, self.y_finetune_cost, self.y, self.y_pred],
              updates=updates_y,
              givens={
        ...
        train_z_fn = theano.function(inputs=[index, learning_rate],
              outputs=[self.acc2, self.z_finetune_cost, self.z, self.z_pred],
              updates=updates_z,
              givens={
                self.x: train_set_x[index * batch_size:
                                    (index + 1) * batch_size],
        ...
        train_x_fn = theano.function(inputs=[index, learning_rate],
              outputs=[self.acc, self.acc2, self.x_finetune_cost],
              updates=updates_x,
              givens={
                self.x: train_set_x[index * batch_size:
                                    (index + 1) * batch_size],
