...
class ResNet(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, cfg = None, testing = False, input = None):
        ...
        n = 2
        # first stack of residual blocks, output is 32 x 21 x 20
        for _ in range(n):
            self.network = self.residual_block(self.network)
        
        # second stack of residual blocks, output is 64 x 11 x 10
        self.network = self.residual_block(self.network, increase_dim=True, projection=True)
        for _ in range(1,n):
            self.network = self.residual_block(self.network)
        
        # third stack of residual blocks, output is 128 x 6 x 5
        self.network = self.residual_block(self.network, increase_dim=True, projection=True)
        for _ in range(1,n):
            self.network = self.residual_block(self.network)
