...
class ResNet(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, cfg = None, testing = False, input = None):
        ...
        self.network = ConvLayer(self.network, num_filters=64, filter_size=(5,5), stride=(1,1), 
                        nonlinearity=None, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False)
        self.layers.append(layer_info('conv', (64, 3, 5, 5), 2)) # W, b
        
        n = 3
        # first stack of residual blocks, output is 64 x x
        for _ in range(n):
            self.network = self.residual_block(self.network)
        
        # second stack of residual blocks, output is 128 x x
        self.network = self.residual_block(self.network, increase_dim=True, projection=True)
        for _ in range(1,n):
            self.netwrok = self.residual_block(self.network, add=False)
    ...
    def residual_block(self, l, increase_dim=False, projection=False, add=True):
        ...
        stack_1 = ConvLayer(l, num_filters=out_num_filters, filter_size=(1,1), stride=first_stride, nonlinearity=None, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False)
        if add is True:
        ...
        stack_3 = ConvLayer(stack_2, num_filters=out_num_filters, filter_size=(1,1), stride=(1,1), nonlinearity=None, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False)
        if add is True:
            self.layers.append(layer_info('conv', (out_num_filters, out_num_filters, 3, 3), 2))
        ...
        if increase_dim:
            ...
            block = NonlinearityLayer(ElemwiseSumLayer([stack_2, stack_l]), nonlinearity=rectify)
