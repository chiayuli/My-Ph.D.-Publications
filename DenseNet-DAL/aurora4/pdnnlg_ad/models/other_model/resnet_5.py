...
class ResNet(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, cfg = None, testing = False, input = None):
        ...
        self.network = batch_norm(ConvLayer(self.network, num_filters=128, filter_size=(3,3), stride=(1,1), 
                        nonlinearity=rectify, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False))
        self.layers.append(layer_info('conv-1', (128, 3, 3, 3), 2)) # W, b
        self.layers.append(layer_info('bn', num_params=2))
        
        n = 2
        # first stack of residual blocks, output is 128
        for _ in range(n):
            self.network = self.residual_block(self.network)
            print "[residual_block]", self.network.output_shape , lasagne.layers.get_all_params(self.network, trainable=True)
        
        # second stack of residual blocks, output is 256
        ...
        # third stack of residual blocks, output is 512
        self.network = self.residual_block(self.network, increase_dim=True, projection=True)
        print "[residual_block]", self.network.output_shape, lasagne.layers.get_all_params(self.network, trainable=True)
        for _ in range(1,n):
            self.network = self.residual_block(self.network)
    ...
    def residual_block(self, l, increase_dim=False, projection=False):
        ...
        self.layers.append(layer_info('conv-1', (out_num_filters, input_num_filters, 3, 3), 2))
        self.layers.append(layer_info('bn', num_params=2))

        stack_2 = batch_norm(ConvLayer(stack_1, num_filters=out_num_filters, filter_size=(3,3), stride=(1,1), nonlinearity=None, pad='same', W=lasagne.init.HeNormal(gain='relu'), flip_filters=False))
        self.layers.append(layer_info('conv-1', (out_num_filters, out_num_filters, 3, 3), 2))
        self.layers.append(layer_info('bn', num_params=2))
