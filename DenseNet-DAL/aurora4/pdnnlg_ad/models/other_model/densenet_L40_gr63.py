...
class DENSENET(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, cfg = None, testing = False, input = None):
        ...
        self.network = lasagne.layers.Conv2DLayer(self.network, num_filters=12, filter_size=3, 
        ...
        #print "post_bn "
        #print "Params:", lasagne.layers.get_all_params(self.network, trainable=True)
        ...
        self.network = DenseLayer(self.network, 1024, nonlinearity=rectify, name='affine')
        self.layers.append(layer_info('fc', (cfg.n_ins, 1024), 2))

        #self.network = DenseLayer(self.network, 1024, nonlinearity=rectify, name='affine')
        #self.layers.append(layer_info('fc', (1024, 1024), 2))
        ...
        #print "output fc "
        #print "Params:", lasagne.layers.get_all_params(self.network, trainable=True)
        self.layers.append(layer_info('fc', (1024, self.cfg.n_outs), 2))

        # get params of each layer
        #params = lasagne.layers.get_all_params(self.network)
        #for i in range(0, len(params)):
            #tmp = [params[i]]
        #    self.params.extend(params[i])
    ...
    def dense_block(self, network, num_layers, growth_rate, dropout, name_prefix):
        ...
        for n in range(num_layers):
            ...
            network = ConcatLayer([network, conv], axis=1,
                                  name=nam)
            #print "Concat ", nam
            #print "Params:", lasagne.layers.get_all_params(network, trainable=True)
    ...
    def bn_relu_conv(self, network, channels, filter_size, dropout, name_prefix):
        #print "[bn_relu_conv] ", name_prefix
        nam=name_prefix + '_bn'
    ...
    def build_finetune_functions(self, train_shared_xy, valid_shared_xy, batch_size):
        ...
        #updates = lasagne.updates.adam(self.finetune_cost, self.params, lrate)
        ...
        updates = lasagne.updates.nesterov_momentum(
                self.finetune_cost + l2_loss, self.params, learning_rate=lrate, momentum=0.9)

        train_fn = theano.function(inputs=[index, lrate],
              outputs=[acc],
              updates=updates,
              givens={
