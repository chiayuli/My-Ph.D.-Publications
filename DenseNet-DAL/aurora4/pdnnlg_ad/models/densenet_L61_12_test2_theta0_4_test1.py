...
class DENSENET(object):
    ...
    def build_y_network(self, input_var=None):
        input = input_var.reshape((-1,76,4,19))
        layer = InputLayer(shape=(None, 76, 4, 19), input_var=input)
        num_blocks = 3
        depth = 49 
    ...
    def build_z_network(self, input_var=None):                
        layer = InputLayer(shape=(None, 5776), input_var=input_var)                
        ...
        self.z_layers.append(layer_info('fc', (5776, 512), 2))
        #layer = DenseLayer(layer, 1024, nonlinearity=sigmoid)                
