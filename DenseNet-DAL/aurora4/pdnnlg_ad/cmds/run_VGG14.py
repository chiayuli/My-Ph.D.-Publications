...
from io_func.model_io_vgg import _nnet2file, _cfg2file, _file2nnet, log
from utils.utils import parse_arguments
from utils.learn_rates import _lrate2file, _file2lrate

from utils.network_config import NetworkConfig
from learning.sgd_vgg import train_sgd, validate_by_minibatch

from models.vgg14 import VGG
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_VGG14.') and os.path.exists(wdir + '/training_state.tmp_VGG14.'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_VGG14.')
        log('> ... found nnet.tmp_VGG14. and training_state.tmp_VGG14., now resume training from epoch ' + str(cfg.lrate.epoch))

    numpy_rng = numpy.random.RandomState(89677)
    theano_rng = RandomStreams(numpy_rng.randint(2 ** 30))
    log('> ... initializing the model')
    # construct the cnn architecture
    vgg = VGG(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = cfg)
    # load the pre-training networks, if any, for parameter initialization
    if resume_training:
        _file2nnet(vgg, filename = wdir + '/nnet.tmp_VGG14.')

    # get the training, validation and testing function for the model
    log('> ... getting the finetuning functions')
    train_fn, valid_fn = vgg.build_finetune_functions(
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(vgg, filename=wdir + '/nnet.tmp_VGG14.')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_VGG14.')

    # save the model and network configuration
    if cfg.param_output_file != '':
        _nnet2file(vgg, filename=cfg.param_output_file, input_factor = cfg.input_dropout_factor, factor = cfg.dropout_factor)
        log('> ... the final PDNN model parameter is ' + cfg.param_output_file)
    if cfg.cfg_output_file != '':
        _cfg2file(vgg.cfg, filename=cfg.cfg_output_file)
    ...
    if cfg.kaldi_output_file != '':
        vgg.write_model_to_kaldi(cfg.kaldi_output_file)
        log('> ... the final Kaldi model (only FC layers) is ' + cfg.kaldi_output_file)

    # remove the tmp_VGG14. files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_VGG14.')
    #os.remove(wdir + '/training_state.tmp_VGG14.')
