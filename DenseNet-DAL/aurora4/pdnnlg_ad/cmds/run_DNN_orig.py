...
from models.dnn_orig import DNN

from io_func.model_io_orig import _nnet2file, _cfg2file, _file2nnet, log
from utils.utils import parse_arguments
from utils.learn_rates import _lrate2file, _file2lrate

from utils.network_config_orig import NetworkConfig
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DNN_orig') and os.path.exists(wdir + '/training_state.tmp_DNN_orig'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DNN_orig')
        log('> ... found nnet.tmp_DNN_orig and training_state.tmp_DNN_orig, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(dnn, filename = wdir + '/nnet.tmp_DNN_orig')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(dnn, filename=wdir + '/nnet.tmp_DNN_orig')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DNN_orig') 
    ...
    # remove the tmp_DNN_orig files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DNN_orig')
    #os.remove(wdir + '/training_state.tmp_DNN_orig') 
