...
if __name__ == '__main__':
    ...
    #if os.path.exists(wdir + '/nnet.tmp_CNN.1') and os.path.exists(wdir + '/training_state.tmp_CNN.1'):
    #    resume_training = True
    #    cfg.lrate = _file2lrate(wdir + '/training_state.tmp_CNN.1')
    #    log('> ... found nnet.tmp_CNN.1 and training_state.tmp_CNN.1, now resume training from epoch ' + str(cfg.lrate.epoch))

    #numpy_rng = numpy.random.RandomState(89677)
    #theano_rng = RandomStreams(numpy_rng.randint(2 ** 30))
    log('> ... initializing the model cnn1 and cnn2')
    # construct the cnn architecture
    cnn1 = CNN1(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = cfg)
    ...
    _file2nnet(cnn1, filename = wdir + '/nnet.tmp_CNN1')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(cnn, filename=wdir + '/nnet.tmp_CNN.1')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_CNN.1')
    ...
    # remove the tmp_CNN.1 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_CNN.1')
    #os.remove(wdir + '/training_state.tmp_CNN.1')
