...
from io_func.model_io_resnet_ctc import _nnet2file, _cfg2file, _file2nnet, log
...
from models.resnet_ctc import ResNet_ctc
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_ResNet_ctc_2') and os.path.exists(wdir + '/training_state.tmp_ResNet_ctc_2'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_ResNet_ctc_2')
        log('> ... found nnet.tmp_ResNet_ctc_2 and training_state.tmp_ResNet_ctc_2, now resume training from epoch ' + str(cfg.lrate.epoch))

    numpy_rng = numpy.random.RandomState(89677)
    theano_rng = RandomStreams(numpy_rng.randint(2 ** 30))
    log('> ... initializing the model')
    # construct the cnn architecture
    resnet_ctc = ResNet_ctc(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = cfg)
    # load the pre-training networks, if any, for parameter initialization
    if resume_training:
        _file2nnet(resnet_ctc, filename = wdir + '/nnet.tmp_ResNet_ctc_2')

    # get the training, validation and testing function for the model
    log('> ... getting the finetuning functions')
    train_fn, valid_fn = resnet_ctc.build_finetune_functions(
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(resnet_ctc, filename=wdir + '/nnet.tmp_ResNet_ctc_2')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_ResNet_ctc_2')

    # save the model and network configuration
    if cfg.param_output_file != '':
        _nnet2file(resnet_ctc, filename=cfg.param_output_file, input_factor = cfg.input_dropout_factor, factor = cfg.dropout_factor)
        log('> ... the final PDNN model parameter is ' + cfg.param_output_file)
    if cfg.cfg_output_file != '':
        _cfg2file(resnet_ctc.cfg, filename=cfg.cfg_output_file)
    ...
    if cfg.kaldi_output_file != '':
        resnet_ctc.write_model_to_kaldi(cfg.kaldi_output_file)
        log('> ... the final Kaldi model (only FC layers) is ' + cfg.kaldi_output_file)

    # remove the tmp_ResNet_ctc_2 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_ResNet_ctc_2')
    #os.remove(wdir + '/training_state.tmp_ResNet_ctc_2')
