...
from utils.network_config_128 import NetworkConfig
from learning.sgd import train_sgd, validate_by_minibatch

from models.densenet_L40_nobn import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40.nobn.1.128') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40.nobn.1.128'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40.nobn.1.128')
        log('> ... found nnet.tmp_DENSENET.L40.nobn.1.128 and training_state.tmp_DENSENET.L40.nobn.1.128, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40.nobn.1.128')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.nobn.1.128')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.nobn.1.128')
    ...
    # remove the tmp_DENSENET.L40.nobn.1.128 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L40.nobn.1.128')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L40.nobn.1.128')
