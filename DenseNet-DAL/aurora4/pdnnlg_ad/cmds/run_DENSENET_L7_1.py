...
from models.densenet_L7 import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L7.1') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L7.1'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L7.1')
        log('> ... found nnet.tmp_DENSENET.L7.1 and training_state.tmp_DENSENET.L7.1, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L7.1')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L7.1')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L7.1')
    ...
    # remove the tmp_DENSENET.L7.1 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L7.1')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L7.1')
