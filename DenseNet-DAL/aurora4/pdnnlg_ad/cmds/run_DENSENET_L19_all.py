...
from models.densenet_L19 import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L19.all') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L19.all'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L19.all')
        log('> ... found nnet.tmp_DENSENET.L19.all and training_state.tmp_DENSENET.L19.all, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L19.all')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L19.all')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L19.all')
    ...
    # remove the tmp_DENSENET.L19.all files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L19.all')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L19.all')
