...
from learning.sgd_l40_2 import train_sgd, validate_by_minibatch

from models.densenet_L40 import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40.all.ep3') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40.all.ep3'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40.all.ep3')
        log('> ... found nnet.tmp_DENSENET.L40.all.ep3 and training_state.tmp_DENSENET.L40.all.ep3, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40.all.ep3')
    ...
    while (cfg.lrate.get_rate() != 0):
        log('now epoch is ' + str(cfg.lrate.epoch))
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            log('Save nnet params of epoch ' + str(cfg.lrate.epoch))
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.all.test1.ep' + str(cfg.lrate.epoch))
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.all.test1')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.all.test1')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.all.test1.ep' + str(cfg.lrate.epoch))
    ...
    # remove the tmp_DENSENET.L40.all files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L40.all')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L40.all')
