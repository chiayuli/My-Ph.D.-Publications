...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.1.test') and os.path.exists(wdir + '/training_state.tmp_DENSENET.1.test'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.1.test')
        log('> ... found nnet.tmp_DENSENET.1.test and training_state.tmp_DENSENET.1.test, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.1.test')
    ...
    #    i += 1
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.1.test')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.1.test')
    ...
    # remove the tmp_DENSENET.1.test files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.1.test')
    #os.remove(wdir + '/training_state.tmp_DENSENET.1.test')
