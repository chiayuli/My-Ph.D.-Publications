...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40.all.retest') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40.all.retest'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40.all.retest')
        log('> ... found nnet.tmp_DENSENET.L40.all.retest and training_state.tmp_DENSENET.L40.all.retest, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40.all.retest')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.all.retest')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.all.retest')
    ...
    # remove the tmp_DENSENET.L40.all.retest files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L40.all.retest')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L40.all.retest')
