...
from io_func.model_io_lace_bn import _nnet2file, _cfg2file, _file2nnet, log
...
from models.cnn_lace_bn import CNN_LACE
...
if __name__ == '__main__':
    ...
    cfg = NetworkConfig(); cfg.model_type = 'CNN_LACE'
    ...
    if os.path.exists(wdir + '/nnet.tmp_CNN_LACE_bn') and os.path.exists(wdir + '/training_state.tmp_CNN_LACE_bn'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_CNN_LACE_bn')
        log('> ... found nnet.tmp_CNN_LACE_bn and training_state.tmp_CNN_LACE_bn, now resume training from epoch ' + str(cfg.lrate.epoch))

    numpy_rng = numpy.random.RandomState(89677)
    theano_rng = RandomStreams(numpy_rng.randint(2 ** 30))
    log('> ... initializing the model')
    # construct the cnn architecture
    cnn = CNN_LACE(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = cfg)
    # load the pre-training networks, if any, for parameter initialization
    if resume_training:
        _file2nnet(cnn, filename = wdir + '/nnet.tmp_CNN_LACE_bn')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(cnn, filename=wdir + '/nnet.tmp_CNN_LACE_bn')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_CNN_LACE_bn')
    ...
    # remove the tmp_CNN_LACE_bn files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_CNN_LACE_bn')
    #os.remove(wdir + '/training_state.tmp_CNN_LACE_bn')
