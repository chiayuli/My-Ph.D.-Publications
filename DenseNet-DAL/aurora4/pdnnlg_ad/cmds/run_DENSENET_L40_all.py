...
from learning.sgd_l40_1 import train_sgd, validate_by_minibatch
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40.all') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40.all'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40.all')
        log('> ... found nnet.tmp_DENSENET.L40.all and training_state.tmp_DENSENET.L40.all, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40.all')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.all.ep' + str(cfg.lrate.epoch))
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40.all')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.all')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40.all.ep' + str(cfg.lrate.epoch))
