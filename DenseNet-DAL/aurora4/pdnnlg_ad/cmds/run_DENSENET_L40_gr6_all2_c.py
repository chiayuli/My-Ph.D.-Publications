...
from learning.sgd_c22 import train_sgd, validate_by_minibatch
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40_gr6.all2') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40_gr6.all2'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40_gr6.all2')
        log('> ... found nnet.tmp_DENSENET.L40_gr6.all2 and training_state.tmp_DENSENET.L40_gr6.all2, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40_gr6.all2')
    ...
    while (cfg.lrate.epoch < 50):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40_gr6.all2')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40_gr6.all2')
    ...
    # remove the tmp_DENSENET.L40gr6.all2 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L40gr6.all2')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L40gr6.all2')
