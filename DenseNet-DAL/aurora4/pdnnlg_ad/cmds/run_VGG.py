...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_VGG.') and os.path.exists(wdir + '/training_state.tmp_VGG.'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_VGG.')
        log('> ... found nnet.tmp_VGG. and training_state.tmp_VGG., now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(vgg, filename = wdir + '/nnet.tmp_VGG.')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(vgg, filename=wdir + '/nnet.tmp_VGG.')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_VGG.')
    ...
    # remove the tmp_VGG. files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_VGG.')
    #os.remove(wdir + '/training_state.tmp_VGG.')
