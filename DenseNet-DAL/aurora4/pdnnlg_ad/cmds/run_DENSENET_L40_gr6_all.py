...
from learning.sgd_2_all import train_sgd, validate_by_minibatch

from models.densenet_L40_gr6 import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L40_gr7.1.all') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L40_gr7.1.all'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L40_gr7.1.all')
        log('> ... found nnet.tmp_DENSENET.L40_gr7.1.all and training_state.tmp_DENSENET.L40_gr7.1.all, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L40_gr7.1.all')
    ...
    while (cfg.lrate.epoch < 50):
        # one epoch of sgd training
        train_acc = train_sgd(train_fn, cfg)
        log('> epoch %d, lrate (0.00001) %f, training accuracy %f' % (cfg.lrate.epoch, cfg.lrate.get_rate(), 100*numpy.mean(train_acc)) + '(%)')
        # validation 
        valid_acc, valid_error = validate_by_minibatch(valid_fn, cfg)
        log('> epoch %d, lrate (0.00001) %f, validate accuracy %f' % (cfg.lrate.epoch, cfg.lrate.get_rate(), 100*numpy.mean(valid_acc)) + '(%)')
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L40_gr7.1.all')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L40_gr7.1.all')
    ...
    # remove the tmp_DENSENET.L40_gr7.1 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L40_gr7.1')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L40_gr7.1')
