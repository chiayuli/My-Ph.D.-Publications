...
from models.densenet_L81_12_test2 import DENSENET
...
if __name__ == '__main__':
    ...
    if os.path.exists(wdir + '/nnet.tmp_DENSENET.L81_12_test2') and os.path.exists(wdir + '/training_state.tmp_DENSENET.L81_12_test2'):
        resume_training = True
        cfg.lrate = _file2lrate(wdir + '/training_state.tmp_DENSENET.L81_12_test2')
        log('> ... found nnet.tmp_DENSENET.L81_12_test2 and training_state.tmp_DENSENET.L81_12_test2, now resume training from epoch ' + str(cfg.lrate.epoch))
    ...
    if resume_training:
        _file2nnet(densenet, filename = wdir + '/nnet.tmp_DENSENET.L81_12_test2')
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        log('> epoch %d, lrate %f, training accuracy %f' % (cfg.lrate.epoch, cfg.lrate.get_rate(), 81*numpy.mean(train_acc)) + '(%)')
        # validation 
        valid_acc, valid_error = validate_by_minibatch(valid_fn, cfg)
        log('> epoch %d, lrate %f, validate accuracy %f' % (cfg.lrate.epoch, cfg.lrate.get_rate(), 81*numpy.mean(valid_acc)) + '(%)')
        cfg.lrate.get_next_rate(current_error = 81 * numpy.mean(valid_error))

        # output nnet parameters and lrate, for training resume
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(densenet, filename=wdir + '/nnet.tmp_DENSENET.L81_12_test2')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp_DENSENET.L81_12_test2')
    ...
    # remove the tmp_DENSENET.L81_12_test2 files (which have been generated from resuming training) 
    #os.remove(wdir + '/nnet.tmp_DENSENET.L81_12_test2')
    #os.remove(wdir + '/training_state.tmp_DENSENET.L81_12_test2')
