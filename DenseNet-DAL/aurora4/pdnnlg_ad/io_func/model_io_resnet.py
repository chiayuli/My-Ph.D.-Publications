...
def _nnet2file(model, set_layer_num = -1, filename='nnet.out', start_layer = 0, input_factor = 0.0, factor=[]):
    ...
    for i in range(len(layers)):
       ...
       if layers[i].type == 'fc':
           ...
           index += 1
           dict_a = 'b' + str(i)
           print "[Debug-fc] b.shape: ", pp[index].shape
           ...
           dict_a = 'b' + str(i)
           print "[Debug-conv] b.shape: ", pp[index].shape
           nnet_dict[dict_a] = array_2_string(pp[index])
           index += 1

       elif layers[i].type == 'conv-1':
           # conv.W 
           dict_a = 'W' + str(i)
           print "[Debug-conv-1] w.shape: ", pp[index].shape
           for next_X in xrange(pp[index].shape[0]):
               for this_X in xrange(pp[index].shape[1]):
                   new_dict_a = dict_a + ' ' + str(next_X) + ' ' + str(this_X)
                   nnet_dict[new_dict_a] = array_2_string(pp[index][next_X, this_X])
           ...
           dict_a = 'beta' + str(i)
           print "[Debug-bn] beta.shape: ", pp[index].shape
           nnet_dict[dict_a] = array_2_string(pp[index])
           index += 1

           dict_a = 'gamma' + str(i)
           print "[Debug-bn] gamma.shape: ", pp[index].shape
           nnet_dict[dict_a] = array_2_string(pp[index])
           index += 1
