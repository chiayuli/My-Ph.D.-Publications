...
class PfileDataRead(object):
    ...
    def load_next_partition(self, shared_xyz):
        feat = self.feat_mats[self.partition_index]
        label = self.label_vecs[self.partition_index]
        noise = self.noise_vecs[self.partition_index]
        shared_x, shared_y, shared_z = shared_xyz
        ...
        shared_z.set_value(noise.astype(theano.config.floatX), borrow = True)
    ...
    def make_shared(self):
        ...
        noise = self.noise_vecs[0].astype(theano.config.floatX)

        shared_x = theano.shared(feat, name = 'x', borrow = True)
        shared_y = theano.shared(label, name = 'y', borrow = True)
        shared_z = theano.shared(noise, name = 'z', borrow = True)
...
class PfileDataReadStream(object):
    ...
    def __init__(self, pfile_path_list, read_opts):
        ...
        self.noise = numpy.zeros(10, dtype = numpy.int32)

        self.end_reading = False
        #print "self.pfile_path_list", self.pfile_path_list
    ...
    def initialize_read(self, first_time_reading = False):
        ...
        #print "self.pfile_path:", self.pfile_path
        #print "self.cur_pfile_index:", self.cur_pfile_index
        self.read_pfile_info()
        self.end_reading = False

        if re.match(r'(.*)train_multi_tr95(.*)', self.pfile_path):
            self.noise_file_path = "exp/tri2b_multi_ali_tr95/ali."+str(self.cur_pfile_index+1)+".classes.txt"
            #print "self.noise_file", self.noise_file_path
        else:
            self.noise_file_path = "exp/tri2b_multi_ali_cv05/ali."+str(self.cur_pfile_index+1)+".classes.txt"
            #print "self.noise_file", self.noise_file_path
        ...
        if first_time_reading:
            ...
            self.noise = numpy.zeros(self.frame_per_partition, dtype = numpy.int32)

        self.feat_buffer = None
        self.label_buffer = None
        self.noise_buffer = None

        with open(self.noise_file_path) as f:
            index = 0
            for line in f:
                tmp = line.split(' ')
                self.noise[index:index+int(tmp[1])]=int(tmp[0])
                index = index + int(tmp[1])
        #print self.noise
        #print "self.frame_per_partition: ", self.frame_per_partition
        #print "index", index
    ...
    def load_next_partition(self, shared_xyz):
        ...
        shared_z.set_value(self.noise, borrow = True)
    ...
    def make_shared(self):
        ...
        shared_z = theano.shared(self.noise, name = 'z', borrow = True)
        return shared_x, shared_y, shared_z
