...
class EmbedRNN(torch.nn.Module):
    ...
    def forward(self, xpad, xlen):
        ...
        h = self.embed(xpad)
        hpack = pack_padded_sequence(h, xlen.cpu(), batch_first=True)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args):
       ...
       # subsample info
        ...
        self.enc_vgg = self.enc.enc[0]
        self.enc_rnn=[None]*(args.elayers-1)
        self.enc_merge=[None]*(args.elayers-1)
        for i in range(args.elayers-1):
            self.enc_rnn[i] = getattr(self.enc.enc[1], "birnn%d" % (i))
            self.enc_merge[i] = getattr(self.enc.enc[1], "bt%d" % (i))
        self.enc_common_rnn = getattr(self.enc.enc[1], "birnn%d" % (args.elayers-1))
        self.enc_common_merge = getattr(self.enc.enc[1], "bt%d" % (args.elayers-1))
    ...
    def forward_uncommon(self, xpad, xlen):
        #hpad=xpad
        #hlen=xlen
        hpad, hlen, _ = self.enc_vgg(xpad, xlen)
        for i in range(len(self.enc_rnn)):
            hpack = pack_padded_sequence(hpad, hlen.cpu(), batch_first=True)
            hpack, states = self.enc_rnn[i](hpack)
            hpad, hlen = pad_packed_sequence(hpack, batch_first=True)
            b, t, o = hpad.shape
            hpad = torch.tanh(self.enc_merge[i](hpad.contiguous().view(b * t, o)).view(b, t, -1))
        return hpad, hlen
    ...
    def forward(self, xs_pad, ilens, ys_pad, olens, supervised=False, discriminator=None, only_encoder=False):
        ...
        #xs_pad, ilens, ys_pad, olens = input_all
        # 0. Frontend
        ...
        if supervised or not self.training:
            ...
            if False:
                ...
                loss_cyc_h_from_y = l1(hypad,hypad2)
                ...
                #hxpad4, hxlens4 = self.forward_uncommon(xs_pad.to('cuda:0'), ilens)
                ...
                ### high-level representation from text (eq 11)
                hypad, hylens = self.enc_t(ys_pad, olens)
                ys_hat = [to_device(self, to_torch_tensor(yy).long()) for yy in ys_hat]
                ys_hat_pad = pad_list(ys_hat, 0)
                hypad2, hylens2 = self.enc_t(ys_hat_pad, hylens)
                loss_e_cyc_i_from_y = l1(hypad,hypad2)
                #logging.warning("eq 11. loss_e_cyc_i_from_y.mean() %f"%(loss_e_cyc_i_from_y.mean()))

            # 5. identity loss
            l1=torch.nn.SmoothL1Loss()
            loss_idt = 0
            if True :
                ### identity loss for Encoder 
                # when input is text (y), i.e. h=hatE(g(y)), hatE(h)=h
                ...
                loss_idt_enc_t = l1(hypad6,hypad5)
                #logging.warning("loss_idt_enc_t.mean() %f",loss_idt_enc_t.mean())

                # when input is speech (x), i.e. h=E(f(x)), hatE(h)=h
                hxpad7, hxlens7, _ = self.enc(xs_pad, ilens) # Enc(s): h
                hypad8, hylens8 = self.forward_common(hxpad7, hxlens7)
                loss_idt_enc_s = l1(hypad8,hxpad7)
                #logging.warning("loss_idt_enc_s.mean() %f",loss_idt_enc_s.mean())

            #loss = (self.speech_text_ratio * (loss_cyc_h_from_x+loss_idt_enc_s)) +\
            #       ((1.0 - self.speech_text_ratio) * (loss_cyc_h_from_y+loss_idt_enc_t+loss_e_cyc_i_from_y))
            loss=[]
            speech_text_ratio=0.0
            for i in range(11):
              loss.append((speech_text_ratio*(loss_idt_enc_s))+((1.0 - speech_text_ratio)*(loss_text+loss_idt_enc_t)))
              speech_text_ratio=speech_text_ratio+0.1
            return sorted(loss)[0]
