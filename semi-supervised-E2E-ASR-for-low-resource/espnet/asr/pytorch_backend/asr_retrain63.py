...
from espnet.asr.pytorch_backend.asr_init import load_init_model
from espnet.asr.pytorch_backend.asr_init import load_trained_modules
import espnet.lm.pytorch_backend.extlm as extlm_pytorch
from espnet.nets.asr_interface import ASRInterface
from espnet.nets.beam_search_transducer import BeamSearchTransducer
from espnet.nets.pytorch_backend.e2e_asr import pad_list
from espnet.nets.pytorch_backend.unsupervised52 import E2E, Discriminator
...
def load_pretrained(self, src_dict, idim, odim, args):
...
class CustomEvaluator(BaseEvaluator):
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in it:
                x, il, y, ol = _recursive_to(batch, self.device)
                observation = {}
                with reporter_module.report_scope(observation):
                    ...
                    if self.ngpu == 0:
                        self.model(*x, *il, *y, *ol, True)
                    else:
                        # apex does not support torch.nn.DataParallel
                        data_parallel(self.model, (x, il, y, ol, True), range(self.ngpu))
...
class CustomUpdater(StandardUpdater):
    ...
    def __init__(
        ...
        self.supervised_loss_ratio = supervised_loss_ratio
        self.iteration = 0
    ...
    def update_core(self):
        ...
        usup_train_iter = self.get_iterator("unsupervised")
        optimizer = self.get_optimizer("main")
        #d_optimizer = self.get_optimizer("secondary")
        epoch = train_iter.epoch
        #logging.warning("epoch: %d" %(epoch))
        # Get the next batch (a list of json files)
        batch = train_iter.next()
        u_batch = usup_train_iter.next()
        # self.iteration += 1 # Increase may result in early report,
        # which is done in other place automatically.
        x, il, y, ol = _recursive_to(batch, self.device)
        u_x, u_il, u_y, u_ol = _recursive_to(u_batch, 'cuda:0')
        #logging.warning("ubatch: %s %s %s " %(u_il, u_y, u_ol))
        ...
        if self.ngpu == 1:
          loss_supervised = self.model(x,il,y,ol, True).mean() / self.accum_grad
          loss_unsupervised = self.model(u_x,u_il,u_y,u_ol, False).mean() / self.accum_grad
        else:
            # apex does not support torch.nn.DataParallel
            loss_supervised = (
                data_parallel(self.model.to('cuda:0'), (x,il,y,ol,True), range(self.ngpu)).mean() / self.accum_grad
            )
            loss_unsupervised = (
                data_parallel(self.model.to('cuda:0'), (u_x.to('cuda:0'), u_il.to('cuda:0'), u_y.to('cuda:0'), u_ol.to('cuda:0'), False), range(self.ngpu)).mean() / self.accum_grad
            )

        if train_iter.epoch < 2:
          self.supervised_loss_ratio=0.9
        elif train_iter.epoch < 4:
          self.supervised_loss_ratio=0.8
        elif train_iter.epoch < 5:
          self.supervised_loss_ratio=0.7
        elif train_iter.epoch < 6:
          self.supervised_loss_ratio=0.6
        else:
          self.supervised_loss_ratio=0.5
        loss = self.supervised_loss_ratio * loss_supervised + (1.0 - self.supervised_loss_ratio) * (loss_unsupervised)
        loss.backward()
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # get batch of lengths of input/output sequences
...
class CustomConverterU(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        ys = [torch.from_numpy(np.array(ys[i][0][:]) if isinstance(ys[i], tuple) else ys[i]).long() for i in y_sorted_index]
        ypad = pad_list(ys, 0).to(device)
        olens = np.array([olens[i] for i in y_sorted_index])
        olens = torch.from_numpy(olens).to(device)

        return xs_pad, ilens, ypad, olens
...
def train(args):
    ...
    model = E2E(idim_list[0] if args.num_encs == 1 else idim_list, odim, args)
    ...
    if args.opt == "adadelta":
        optimizer = torch.optim.Adadelta(
            model_params, lr=args.learning_rate , rho=0.95, eps=args.eps, weight_decay=args.weight_decay
        )
        if discriminator:
            d_optimizer = torch.optim.Adadelta(
                discriminator.parameters(), rho=0.95, eps=args.eps, weight_decay=args.weight_decay)
    elif args.opt == "adam":
        optimizer = torch.optim.Adam(model_params, lr=args.learning_rate ,weight_decay=args.weight_decay)
        if discriminator:
            d_optimizer = torch.optim.Adam(discriminator.parameters(), weight_decay=args.weight_decay)
    else:
    ...
    if args.num_encs == 1:
        ...
        converterU = CustomConverterU(subsampling_factor=model.subsample[0], dtype=dtype)
    else:
    ...
    supervised_train_batch = make_batchset(
        train_json,
        args.batch_size,
        args.maxlen_in,
        args.maxlen_out,
    ...
    unsupervised_train_batch = make_batchset(
        unsupervised_json,
        args.batch_size,
        args.maxlen_in,
        args.maxlen_out,
    ...
    logging.warning("batch_size %d n_unsupervised %d n_supervised %d" % (args.batch_size, n_unsupervised, n_supervised))

    n_iter_fun = min if args.use_smaller_data_size else max
    if not args.use_smaller_data_size:
        new_supervised_train_batch = []
        for i in range(n_unsupervised):
            idx = i % n_supervised
            new_supervised_train_batch.append(supervised_train_batch[idx])
        supervised_train_batch = new_supervised_train_batch

    #unsupervised_train_batch = shuffle_pair(unsupervised_train_batch)[:n_iter_fun(n_supervised,n_unsupervised)]
    logging.warning("supervised_train_batch: %s" %(len(supervised_train_batch)))
    logging.warning("unsupervised_train_batch: %s" %(len(unsupervised_train_batch)))
    ...
    train_iter = ChainerDataLoader(
        dataset=TransformDataset(supervised_train_batch, lambda data: converter([load_tr(data)])),
        batch_size=1,
        num_workers=args.n_iter_processes,
        shuffle=False,
        collate_fn=lambda x: x[0],
    )
    unsupervised_train_iter = ChainerDataLoader(
        dataset=TransformDataset(unsupervised_train_batch, lambda data: converterU([load_tr(data)])),
        batch_size=1,
        num_workers=args.n_iter_processes,
        shuffle=True,
        collate_fn=lambda x: x[0],
    ...
    valid_iter = ChainerDataLoader(
        dataset=TransformDataset(valid_batch, lambda data: converter([load_cv(data)])),
        batch_size=1,
        shuffle=False,
        collate_fn=lambda x: x[0],
        num_workers=args.n_iter_processes,
    ...
    updater = CustomUpdater(
        model,
        args.grad_clip,
        {"main": train_iter, "unsupervised": unsupervised_train_iter},
        optimizer,
        device,
        args.ngpu,
        args.grad_noise,
        args.accum_grad,
        args.supervised_loss_ratio,
        use_apex=False,
    )
    ...
    #if use_sortagrad:
    trainer.extend(
        ShufflingEnabler([train_iter, unsupervised_train_iter]),
